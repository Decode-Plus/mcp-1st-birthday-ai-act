...
async function scanForAISystems(
	...
	if (!apiKey) {
		console.warn(
			"⚠️  Tavily API key not provided (neither user key nor TAVILY_API_KEY env var), using AI model for AI systems discovery",
		);
...
async function extractAISystemsFromResults(
	...
	if (userSpecifiedSystems && userSpecifiedSystems.length > 0) {
		console.error(
			`🎯 Filtering for user-specified systems: ${userSpecifiedSystems.join(", ")}`,
		);
	}
	...
	const systemPatterns = [
		// ANNEX III POINT 8(a) - LEGAL/JUDICIAL AI (HIGH RISK)
		{
			...
			description:
				"AI system for legal assistance, advice, research, or document analysis",
			intendedPurpose:
				"Legal consulting, contract review, legal research, compliance advice, or interpretation of laws and regulations",
		},
		...
		// ANNEX III POINT 6 - LAW ENFORCEMENT (HIGH RISK)
		{
			...
			intendedPurpose:
				"Automated detection of fraudulent transactions and security threats",
		},
		// ANNEX III POINT 5(b) - HEALTHCARE (HIGH RISK)
		{
			...
			intendedPurpose:
				"Medical diagnosis support, patient data analysis, or clinical decision support",
		},
		// ANNEX III POINT 5(b) - CREDIT SCORING (HIGH RISK)
		{
			...
			intendedPurpose:
				"Automated credit evaluation and loan approval recommendations",
		},
		// ANNEX III POINT 1 - BIOMETRICS (HIGH RISK)
		{
			...
			intendedPurpose:
				"Biometric authentication, facial recognition, or identity verification",
		},
		// ANNEX III POINT 3 - EDUCATION (HIGH RISK)
		{
			...
			intendedPurpose:
				"Student evaluation, grading, admissions decisions, or learning outcome assessment",
		},
		// LIMITED RISK - Chatbots (Article 50)
		{
			...
			intendedPurpose:
				"Automated customer support, FAQ responses, and order status inquiries",
		},
		// MINIMAL RISK
		{
			keywords: [
			...
			intendedPurpose:
				"Product or content recommendations based on user behavior and preferences",
		},
		{
	...
	for (const pattern of systemPatterns) {
		...
		if (userSpecifiedSystems && userSpecifiedSystems.length > 0) {
			matchesUserRequest = userSpecifiedSystems.some((userSystem) => {
				...
				return (
					pattern.systemType.toLowerCase().includes(userSystemLower) ||
					userSystemLower.includes(
						pattern.systemType.toLowerCase().split(" ")[0],
					) ||
		...
		if (hasKeywords && matchesUserRequest) {
			...
			console.error(
				`⊘ Found ${pattern.systemType} but doesn't match user-specified systems, skipping...`,
			);
		}
	}
	...
	if (
		...
		for (const userSystem of userSpecifiedSystems) {
			...
			if (
				allContent.includes(userSystemLower) ||
...
function createCustomSystemProfile(
	...
	if (
		...
		systemName.toLowerCase().includes("recruit")
	) {
		riskCategory = "High";
		systemType = systemName.includes("AI")
			? systemName
			: `${systemName} AI Assistant`;
	} else if (
		content.includes("healthcare") ||
		content.includes("medical") ||
		systemName.toLowerCase().includes("health")
	) {
		riskCategory = "High";
		systemType = systemName.includes("AI")
			? systemName
		...
		systemName.toLowerCase().includes("credit")
	) {
		riskCategory = "High";
		systemType = systemName.includes("System")
			? systemName
			: `${systemName} System`;
	} else if (
		content.includes("biometric") ||
		systemName.toLowerCase().includes("biometric") ||
		systemName.toLowerCase().includes("facial")
	) {
		riskCategory = "High";
		systemType = systemName.includes("System")
			? systemName
			: `${systemName} Identification System`;
	} else if (
		content.includes("chatbot") ||
		content.includes("conversational") ||
		systemName.toLowerCase().includes("chatbot") ||
		systemName.toLowerCase().includes("chat")
	) {
		riskCategory = "Limited";
		systemType = systemName.includes("Chatbot")
			? systemName
			: `${systemName} Chatbot`;
	}
...
function createAISystemProfile(
	...
	const riskClassification = buildRiskClassification(
...
function validateRiskClassificationConsistency(riskClassification: any): any {
	...
	// NOTE: We do NOT classify systems as "Unacceptable" in this discovery tool.
	// Unacceptable risk classification (Article 5 prohibited practices) is handled
	// by assess-compliance.ts, not here. This function only validates High, Limited, or Minimal.

	// Determine if there's a mismatch
	let correctedCategory = rc.category;

	// If system is marked as Unacceptable, downgrade to High for discovery phase
	// (assess-compliance.ts will properly classify as Unacceptable if needed)
	if (rc.category === "Unacceptable") {
		correctedCategory = "High";
		console.error(
			`[Risk Validation] Downgraded "Unacceptable" to "High" - unacceptable classification handled by assess-compliance.ts`,
		);
	} else if (
		(hasAnnexIIIIndicator ||
			hasHighRiskJustification ||
			hasHighRiskScore ||
			requiresConformity) &&
		rc.category !== "High" &&
	...
	if (correctedCategory !== rc.category) {
		...
		if (hasAnnexIIIIndicator)
			console.error(
				`  - Reason: annexIIICategory indicates "${rc.annexIIICategory}"`,
			);
		if (hasHighRiskScore)
			console.error(`  - Reason: riskScore ${rc.riskScore} >= 70`);
		if (requiresConformity)
			console.error(`  - Reason: conformityAssessmentRequired is true`);

		rc.category = correctedCategory;

		// Also ensure other fields are consistent with the corrected category
		if (correctedCategory === "High") {
			rc.conformityAssessmentRequired = true;
			if (
				!rc.conformityAssessmentType ||
...
function buildRiskClassification(
	...
	if (category === "Limited") {
		return {
			...
			justification:
				"Customer-facing AI system requiring mandatory transparency disclosure per Article 50(1). Does not fall under Annex III high-risk categories.",
			riskScore: 25,
	...
	return {
		...
		justification:
			"AI system does not fall under high-risk or limited-risk categories. No specific EU AI Act obligations beyond general requirements.",
		riskScore: 10,
		conformityAssessmentRequired: false,
	...
	if (systemType.includes("Recruitment")) {
		return {
			...
			humanOversight: {
				enabled: true,
				description:
					"HR managers review AI recommendations for final decisions",
			},
		};
	}
...
async function discoverAISystemsWithAI(
	...
	const organizationName =
		orgContext?.organization?.name || "Unknown Organization";
	const model = getModel(modelName, apiKeys, "discover_ai_services");

	const systemNamesContext =
		systemNames && systemNames.length > 0
...
IMPORTANT: Do NOT classify systems as "Unacceptable" in this discovery phase. Unacceptable risk classification (prohibited practices per Article 5) will be determined during compliance assessment.
...
[
  {
    ...
    "riskCategory": "<High|Limited|Minimal>",
...
Provide 1-5 AI systems based on what you know about ${organizationName}. If you don't have specific knowledge, infer based on their industry sector (${orgContext?.organization?.sector || "Technology"}).`;
	...
	try {
		...
		console.error(
			`✅ AI model discovered ${parsedSystems.length} AI systems for ${organizationName}`,
		);

		// Convert parsed systems to AISystemProfile format
		return parsedSystems.map((sys, index) => {
			// Convert "Unacceptable" to "High" - unacceptable classification is handled by assess-compliance.ts
			let riskCategory = (sys.riskCategory || "Minimal") as RiskCategory;
			if (riskCategory === "Unacceptable") {
				console.warn(
					`⚠️  System "${sys.name}" was classified as "Unacceptable" by AI model. Converting to "High" - unacceptable classification will be handled by assess-compliance.ts`,
				);
				riskCategory = "High";
			}
			const riskClassification = buildRiskClassification(
	...
	return [
		{
			system: {
				...
				intendedPurpose:
					"Unable to discover specific AI systems. Please provide system names manually.",
				version: "1.0.0",
				status: "Production",
				provider: {
			...
			riskClassification: {
				...
				justification:
					"Risk classification pending - manual system specification recommended",
				safetyComponent: false,
				riskScore: 10,
				conformityAssessmentRequired: false,
				conformityAssessmentType: "Not Required",
			...
			complianceStatus: {
				...
				hasAutomatedLogging: false,
				identifiedGaps: [
					"INFO: Specify AI system names for detailed discovery",
				],
				complianceDeadline: "2027-08-02",
 ...
 * NOTE: This function does NOT classify systems as "Unacceptable" - that is handled by assess-compliance.ts
 * This function only classifies as High, Limited, or Minimal risk.
 */
function classifyRisk(system: AISystemProfile): RiskCategory {
	...
	// Unacceptable risk classification is handled by assess-compliance.ts, not here
	...
	if (
		highRiskKeywords.some(
			(keyword) => description.includes(keyword) || purpose.includes(keyword),
		)
	) {
		return "High";
	}
...
export async function discoverAIServices(
	...
	console.error(
		`[discoverAIServices] Starting discovery with: systemNames=${JSON.stringify(systemNames)}, scope=${scope}, context=${context}`,
	);
	...
	if (!tavilyApiKey) {
		...
		console.warn(
			"⚠️  Tavily API key not provided, using AI model for AI systems discovery",
		);
		systems = await discoverAISystemsWithAI(
			organizationContext,
			systemNames,
			model,
			apiKeys,
		...
		try {
			systems = await scanForAISystems(
				organizationContext,
				systemNames,
				tavilyApiKey,
				model,
	...
	const analyzedSystems = systems.map((system) => {
		...
		if (classifiedRisk !== system.riskClassification.category) {
			console.error(
				`[Risk Reclassification] ${system.system.name}: ${system.riskClassification.category} → ${classifiedRisk}`,
			);
			// Rebuild entire risk classification with the new category for consistency
			updatedSystem.riskClassification = buildRiskClassification(
				classifiedRisk,
				system.system.name,
			);
		...
		updatedSystem.riskClassification = validateRiskClassificationConsistency(
			updatedSystem.riskClassification,
		);
	...
	// NOTE: We do not count "Unacceptable" systems here - that classification is handled by assess-compliance.ts
	const riskSummary = {
		unacceptableRiskCount: 0, // Always 0 - unacceptable classification handled by assess-compliance.ts
	...
	const highRiskSystems = analyzedSystems.filter(
		(s) => s.riskClassification.category === "High",
	);
	const limitedRiskSystems = analyzedSystems.filter(
		(s) => s.riskClassification.category === "Limited",
	);
	...
	return {
		...
		complianceSummary: {
			...
			overallCompliancePercentage: Math.round(
				((analyzedSystems.length - complianceSummary.nonCompliantCount) /
					analyzedSystems.length) *
					100,
			),
		},
		regulatoryFramework: {
			...
			jurisdiction:
				"EU-wide (extraterritorial scope for non-EU entities using AI in EU)",
		},
		complianceDeadlines,
		discoverySources: [
		...
		discoveryMetadata: {
			...
			researchIntegration:
				"Tavily-powered compliance research across EU AI Act Articles 6, 9-15, 43, 49-50, 72",
			conformityAssessmentUrgency:
				criticalGaps.length > 0
					? "URGENT: Critical compliance gaps identified"
					: "ACTION REQUIRED: Review gaps before August 2026",
		},
	};
}
